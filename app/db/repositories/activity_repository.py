...
class ActivityRepository:
    ...
    def get_activities_by_bot_id(self, bot_id: int, skip: int = 0, limit: int = 100) -> List[BotActivity]:
        """
        ...
        Returns:
            List of activities
        """
        return self.db.query(BotActivity).filter(BotActivity.bot_id == bot_id).order_by(
            desc(BotActivity.created_at)
    ...
    def get_activities_by_type(self, bot_id: int, activity_type: str, skip: int = 0, limit: int = 100) -> List[BotActivity]:
        """
        ...
        Returns:
            List of activities
        """
        return self.db.query(BotActivity).filter(
            BotActivity.bot_id == bot_id,
            BotActivity.activity_type == activity_type
        ).order_by(desc(BotActivity.created_at)).offset(skip).limit(limit).all()
    
    def get_activities_by_target(self, bot_id: int, target_id: str) -> List[BotActivity]:
        """
        ...
        ).order_by(desc(BotActivity.created_at)).all()
    
    def check_activity_exists(self, bot_id: int, activity_type: str, target_id: str) -> bool:
        """
    ...
    def count_activities_by_bot(self, bot_id: int) -> int:
        ...
        Returns:
            Activity count
        """
        return self.db.query(func.count(BotActivity.id)).filter(BotActivity.bot_id == bot_id).scalar()
    ...
    def count_activities_by_type(self, bot_id: int, activity_type: str) -> int:
        ...
        Returns:
            Activity count
        """
        return self.db.query(func.count(BotActivity.id)).filter(
            BotActivity.bot_id == bot_id,
            BotActivity.activity_type == activity_type
        ).scalar()
    ...
    def get_recent_activities(self, limit: int = 100) -> List[BotActivity]:
        ...
        Returns:
            List of activities
        """
        return self.db.query(BotActivity).order_by(desc(BotActivity.created_at)).limit(limit).all()
