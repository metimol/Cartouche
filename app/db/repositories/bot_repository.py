...
class BotRepository:
    ...
    def get_bots_by_category(self, category: str, skip: int = 0, limit: int = 100) -> List[Bot]:
        """
        ...
        Returns:
            List of bots
        """
        return self.db.query(Bot).filter(Bot.category == category).offset(skip).limit(limit).all()
    
    def get_active_bots(self, hours: int = 24, skip: int = 0, limit: int = 100) -> List[Bot]:
        """
        ...
        return self.db.query(Bot).filter(Bot.last_active >= cutoff).offset(skip).limit(limit).all()
    ...
    def count_bots_by_category(self) -> Dict[str, int]:
        ...
        """
        result = self.db.query(Bot.category, func.count(Bot.id)).group_by(Bot.category).all()
