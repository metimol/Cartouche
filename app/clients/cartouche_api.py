...
class CartoucheAPIClient:
    ...
    async def add_post(self, post_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with self.session.post(url, params=params, json=post_data) as response:
                if response.status == 200:
                    return await response.json()
                else:
                    error_text = await response.text()
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
    async def add_comment(self, post_id: int, comment_data: Dict[str, Any]) -> Dict[str, Any]:
        """
