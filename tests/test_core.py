...
logger.info(
    f"Environment variables: DEFAULT_LLM_PROVIDER={DEFAULT_LLM_PROVIDER}, GOOGLE_API_KEY={GOOGLE_API_KEY[:5]}..."
)
...
async def test_content_generator():
    ...
    try:
        ...
        content_generator = ContentGenerator(
            llm_provider=DEFAULT_LLM_PROVIDER, api_key=GOOGLE_API_KEY
        )

        # Test bot description generation
        description = await content_generator.generate_bot_description(
            "fan", 25, "Male"
        )
        logger.info(f"Generated bot description: {description}")

        # Test comment generation
        comment = await content_generator.generate_comment(
            "fan", "I just got a new car!"
        )
